...
class DiffusionModelEncoder_ansio(nn.Module):
    def __init__(
        ...
        #256*256*32 -> 128*128*16 -> 64*64*16 -> 32*32*16 -> 16*16*16
        # self.out0 = nn.Sequential(normalization(num_channels[0]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((16,16,2)), nn.Flatten(), nn.Linear(num_channels[0]*16*16*2, 512))
        # self.out1 = nn.Sequential(normalization(num_channels[1]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((8,8,1)), nn.Flatten(), nn.Linear(num_channels[1]*8*8*1, 512))
        # self.out2 = nn.Sequential(normalization(num_channels[2]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((4,4,1)), nn.Flatten(), nn.Linear(num_channels[2]*4*4*1, 512))
        # self.out3 = nn.Sequential(normalization(num_channels[3]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((2,2,1)), nn.Flatten(), nn.Linear(num_channels[3]*2*2*1, 512))
        # self.out4 = nn.Sequential(normalization(num_channels[4]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((1,1,1)), nn.Flatten(), nn.Linear(num_channels[4]*1*1*1, 512))
        ...
        self.out4 = nn.Sequential(normalization(num_channels[3]), nn.SiLU(), nn.Dropout(0.1), nn.AdaptiveAvgPool3d((2,2,2)), nn.Flatten(), nn.Linear(num_channels[3]*2*2*2, 512))
    ...
    def forward(
        ...
        # latent_list = [output0, output1, output2]
