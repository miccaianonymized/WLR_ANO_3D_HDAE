...
class DiffusionInferer_ae_seg(Inferer):
    """
    ...
    def __call__(
        ...
        """
        if mode not in ["crossattn", "concat", "encoding", "cat_encoding"]:
            raise NotImplementedError(f"{mode} condition is not supported")

        noisy_image = self.scheduler.add_noise(original_samples=inputs, noise=noise, timesteps=timesteps)
        if mode in ["concat", "cat_encoding"]:
            noisy_image = torch.cat([noisy_image, condition], dim=1)
            condition = None
        prediction = diffusion_model(x=noisy_image, timesteps=timesteps, context=condition, cond=cond)
        ...
        for t in progress_bar:
            ...
            if mode == "encoding":
                model_output = diffusion_model(
                    image, timesteps=torch.Tensor((t,)).to(input_noise.device), cond=cond
                )
            elif mode == "cat_encoding":
                model_input = torch.cat([image, conditioning], dim=1)
                model_output = diffusion_model(
                    model_input, timesteps=torch.Tensor((t,)).to(input_noise.device), cond=cond
                )
