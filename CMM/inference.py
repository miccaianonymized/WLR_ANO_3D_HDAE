...
def main():
    ...
    main_worker_enc_96(args=args)

def save_image_96(args, pred, images_ts, img_path_list, i, times):
    ...
    sitk.WriteImage(save_pred, img_path_list[i].replace("/IMG/", "/IMG_infer/").replace(".nii.gz", f"_96_{str(times)}_pred.nii.gz"))
    sitk.WriteImage(save_ori, img_path_list[i].replace("/IMG/", "/IMG_infer/").replace(".nii.gz", f"_96_ori.nii.gz"))

def main_worker_enc_96(args):
    ...
    loader, img_path_list = get_loader_96(args)
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(loader):
            ...
            for times in infer_time_list:
                ...
                for batch_save in range(args.batch_size):
                    save_image_96(args, image_pred_ema[batch_save:batch_save+1,:,:,:,:], images_ts[batch_save:batch_save+1,:,:,:,:], img_path_list, idx*args.batch_size + batch_save, times)
